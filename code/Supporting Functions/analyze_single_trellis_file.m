...
source_path = ['C:\Users\',username,'\Downloads']; % Set local data path
box_path    = ['C:\Users\',username,'\Box\Data_for_Dylan'];
trial       = 7; % Trial number which matches data file number
chans_to_read = [1:32];
plot_snips         = false; % Plot raw data, STA, and RMS of trial
plot_STA_all_chans = false;
plot_STA_chan      = false;
...
use_trellis_file = true; % choose either to load data from box or cell data table
use_cloud_path  = false; % Choose whether to read from local or cloud data repository

fprintf("rms offset: %f\n", 1000*expmt_list{expmt,1}.nerve_length/30);


verbose = false;
offset_1_chans = [];%[7,24,27];
...
STA_t_max   = 498;
fs          = 30e3;
N_channels  = length(chans_to_read);%32;
...
if use_trellis_file==true
    ...
    tic
    analogData = getAnalogData(expmt_list, expmt, rec_filePath, chans_to_read, use_cloud_path, use_trellis_file);
    toc
end
...
for chan = 1:length(chans_to_read)%1:N_channels
    
    if false%ismember(chan,skip_channels)==true
        a=1;
    else
        ...
        if expmt_list{expmt,1}.manual_rms_offset(chan,trial)==true
            rms_offset = temp_rms_offset+30;% 3ms;
        else
        ...
        [crossed, max_val, max_idx, time_bins, resp_bins] = response_detection(exp_data, threshold, Vrms_list, Vrms_time, rms_offset, bin_time, true,100);
        %fprintf('%d\n',sum(resp_bins))
        ...
        if plot_RMS_all_chans==true
            ...
            plot_RMS_grid(Vrms_list, Vrms_time, titleName, chan, threshold, rms_offset, bin_time, true, 100);
        end
        % STA
        ...
        if plot_snips==true
            separate_snippets=false;
            if separate_snippets==true
                % Plot snippets from channel
                
                snippet = 26;
                plot(STA_time, snipData(snippet,:));
                titleName = sprintf('Trial %d PW%dus %duA Ch9 snip%d', trial, pw*1000, stim, snippet);
                title(titleName);
                ylabel('uV')
                xlabel('time (msec)')
                axis tight;
        ...
        if plot_STA_chan==true
            ...
            plot(STA_time,STA_data, 'LineWidth',0.1);
            title(['Trial ', num2str(trial),' Chan ', num2str(chan),' ', num2str(stim),'mA PW',num2str(pw*1000),'us STA'])
            ylim([-5,5]);
            ylabel('uV')
            xlabel('time (msec)')
        ...
        if plot_raw==true
            figure(100);
            plot(STA_time, snipData(40,:))
            title(['Trial ', num2str(trial),' Chan ', num2str(chan),' ', num2str(stim),'mA PW',num2str(pw*1000),'us Raw Data'])
            %ylim([-5,5]);
            ylabel('uV')
            xlabel('time (msec)')
            xlim([STA_t_min,STA_t_max]);
        ...
        if plot_RMS_chan==true
            ...
            if plot_cv_shades==true
                plot_RMS_figure(Vrms_list, Vrms_time, fig_title, threshold, rms_offset, resp_bins, time_bins, 50);
            else
        ...
        if plot_hist==true
            figure(7)
            hist(Vrms_list(rms_offset:end),25);
            h = findobj(gca,'Type','patch');
            h.FaceColor = [0 0.4470 0.7410];
            %hist(rms_threshold.rms_search_window,100)
            hold on;
            plot(threshold, 1:250, 'r.'); %RMS line plot
            plot(threshold, 1:250, 'r.'); %RMS line plot
            xlabel('Signal Amplitude (uV)');
            ylabel('Frequency');
            title('RMS Signal Histogram')
            hold off;
            drawnow
        end
        if plot_RMS_chan==true || plot_STA_chan==true
            fprintf("Response: %d, ", crossed);
            d=input('Next chan?: ');
        end
    end
